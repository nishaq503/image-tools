...
def replace_extension(name: str) -> str:
    ...
    return name.replace(".ome.tif", constants.POLUS_EXT).replace(
        ".ome.zarr", constants.POLUS_EXT
    )


def count_blocks(
    reader_or_writer: types.ReaderOrWriter, tile_size: typing.Optional[int] = None
) -> int:
